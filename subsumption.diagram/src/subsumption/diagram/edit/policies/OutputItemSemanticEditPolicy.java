...
import subsumption.diagram.edit.commands.OutputActivatesCreateCommand;
import subsumption.diagram.edit.commands.OutputActivatesReorientCommand;
import subsumption.diagram.edit.commands.OutputSinkCreateCommand;
import subsumption.diagram.edit.commands.OutputSinkReorientCommand;
...
public class OutputItemSemanticEditPolicy extends
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			case OutputOutputCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit
					...
					switch (SubsumptionVisualIDRegistry.getVisualID(cnode)) {
					case InhibitorEditPart.VISUAL_ID:
						for (Iterator<?> it = cnode.getTargetEdges().iterator(); it
								.hasNext();) {
							Edge incomingLink = (Edge) it.next();
							if (SubsumptionVisualIDRegistry
									.getVisualID(incomingLink) == OutputActivatesEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(
	...
	protected Command getStartCreateRelationshipCommand(
		...
		if (SubsumptionElementTypes.OutputSink_4001 == req.getElementType()) {
			return getGEFWrapper(new OutputSinkCreateCommand(req,
					req.getSource(), req.getTarget()));
		}
		if (SubsumptionElementTypes.OutputActivates_4002 == req
				.getElementType()) {
			return getGEFWrapper(new OutputActivatesCreateCommand(req,
					req.getSource(), req.getTarget()));
		}
		return null;
	}
	...
	protected Command getReorientReferenceRelationshipCommand(
		...
		case OutputSinkEditPart.VISUAL_ID:
			return getGEFWrapper(new OutputSinkReorientCommand(req));
		case OutputActivatesEditPart.VISUAL_ID:
			return getGEFWrapper(new OutputActivatesReorientCommand(req));
		}
